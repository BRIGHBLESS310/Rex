...
# ========= PUT YOUR NEW KEYS HERE AFTER DELETING OLD LEAKED ONES =========
OPENAI_KEY = "PUT_NEW_OPENAI_KEY_HERE"
ANTHROPIC_KEY = "PUT_NEW_CLAUDE_KEY_HERE"
THIRD_KEY = "PUT_NEW_THIRD_KEY_HERE"
# ==========================================================================
...
HTML = """<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Rex.py 3-Keys - VS Code</title>
...
<div class="sidebar"><h2>🤖 REX.PY 3-KEYS</h2><div style="font-size:10px;opacity:.5">VS Code<br>If one nor work, another go work! hahah</div>
...
<button class="side-btn" id="btn-media" onclick="setMode('media')">📁 Media</button>
<button class="side-btn" id="btn-research" onclick="setMode('research')">🧠 Research</button>
<div class="key-box" id="keyBox"><div style="font-weight:700;margin-bottom:4px">🔑 LIVE LOGS</div><div id="liveLogs" style="font-size:9px;opacity:.7">Ready - 3 keys loaded...</div></div>
<button class="side-btn" onclick="clearChat()" style="margin-top:auto;background:#2a1a1a">🗑️ Clear</button>
</div>
<div class="main"><div class="header"><h1 id="title">💬 rex.py - 3 Keys</h1><span style="font-size:10px;background:#00ff88;color:#000;padding:4px 10px;border-radius:20px;font-weight:700">3 APIs ✅</span></div>
<div class="video-area"><div class="avatar" id="avatar"><img id="avImg" src="https://i.pravatar.cc/300?img=68"></div></div>
<div class="chat" id="chat"><div class="msg bot">Wetin dey sup Blessed? Na rex.py 3-KEYS for VS Code! 😂

1️⃣ Try OpenAI
2️⃣ If fail → Try Claude
3️⃣ If fail → Try Local

If one nor work, another go work! hahah!

SETUP:
1. Delete old leaked keys
2. Create NEW keys and put for top of rex.py
3. Save → python rex.py → localhost:5000

Type "how are you" now! 🚀</div></div>
...
function setMode(m){mode=m;document.querySelectorAll('.side-btn').forEach(b=>b.classList.remove('active'));document.getElementById('btn-'+m)?.classList.add('active');document.getElementById('title').textContent=m.toUpperCase()+' - rex.py 3 KEYS';}
...
def image_route():
    return jsonify({"reply": "🎨 Image prompt ready!", "logs": ["Image mode"]})
...
if __name__ == "__main__":
    print("="*70)
    print("REX.PY 3-KEYS - VS CODE - If one nor work, another go work! hahah")
